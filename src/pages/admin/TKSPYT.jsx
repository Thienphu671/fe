...
import { Line } from "react-chartjs-2"; // ✅ Sửa đổi từ Bar sang Line
import {
  fetchFavoriteYear,
  fetchFavoriteMonthsByYear,
  fetchFavoriteDaysByMonth,
  fetchTotalRevenue,
  fetchFavoriteChart,
} from "../../api/doanhthu";
import {
...
const ThongKeSPYT = () => {
  ...
  useEffect(() => {
    fetchFavoriteYear().then((response) => {
  ...
  useEffect(() => {
    if (selectedYear) {
      fetchFavoriteMonthsByYear(selectedYear).then((res) => setMonths(res.data));
      fetchData();
    }
  }, [selectedYear]);
  ...
  useEffect(() => {
    if (selectedYear && selectedMonth) {
      fetchFavoriteDaysByMonth(selectedYear, selectedMonth).then((res) => setDays(res.data));
      fetchData();
    }
  }, [selectedMonth]);
  ...
  const fetchData = () => {
    ...
    fetchFavoriteChart(selectedYear, selectedMonth, selectedDay).then((res) => {
      const data = res.data;
          ...
          datasets: [
            {
              ...
              data: values,
              fill: false, // Đảm bảo không có màu nền
